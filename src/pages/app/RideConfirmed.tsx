...
export default function RideConfirmed() {
  useEffect(() => {
    ...
    calBtn?.addEventListener('click', handleCal);
    ...
    const printBtn = document.querySelector('button[title="Print Transit Pass"]');
    const handlePrint = () => window.print();
    printBtn?.addEventListener('click', handlePrint);

    return () => {
      calBtn?.removeEventListener('click', handleCal);
...
<button className="flex items-center gap-space-xs px-space-md py-2 rounded-lg bg-surface-container-low hover:bg-surface-container text-on-surface font-label-md text-label-md transition-colors shadow-sm" data-path="communities">
<span className="material-symbols-outlined text-base">arrow_back</span>
        Back to Community
      </button>
<button className="flex items-center gap-space-xs px-space-md py-2 rounded-lg bg-surface-container-low hover:bg-surface-container text-on-surface font-label-md text-label-md transition-colors shadow-sm" id="addToCalBtn">
<span className="material-symbols-outlined text-base text-secondary">calendar_add_on</span>
        Add to Calendar
      </button>
<button className="p-2 rounded-lg bg-surface-container-low hover:bg-surface-container text-on-surface-variant hover:text-on-surface transition-colors shadow-sm" title="Print Transit Pass">
<span className="material-symbols-outlined text-base">print</span>
</button>
</div>
</header>
{/*  Calendar Added Toast (Hidden State)  */}
<div className="hidden transition-all duration-300 ease-out mb-space-md p-space-sm px-space-md rounded-xl bg-surface-container-lowest shadow-md flex items-center justify-between" id="calToast">
...
<button className="px-space-sm py-1 rounded bg-secondary text-on-secondary font-label-sm text-label-sm hover:bg-secondary/90 transition-all shrink-0 flex items-center gap-1" id="copyLinkBtn">
...
<a className="w-full py-2 px-space-md rounded-lg bg-transparent hover:bg-primary-container text-surface-bright font-label-md text-label-md text-center transition-colors" data-path="communities" href="#">
            View Community
          </a>
</div>
</section>
